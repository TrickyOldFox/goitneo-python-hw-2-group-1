...
class Record:
    def __init__(self, name_: str, phones: list[str] = None):
...
def main_assignment_flow():
    book = AddressBook()
    # Create entry for John
    john_record = Record("John")
    john_record.add_phone("1234567890")
    john_record.add_phone("5555555555")

    # Add John entry to address book
    book.add_record(john_record)

    # Create entry for Jane and adding it to address book
    ...
    print(john)  # Out: Contact name: John, phones: 1112223333; 5555555555

    # Find specific phone number for John and then delete it
    found_phone = john.find_phone("5555555555")
    print(f"{john.name}: {found_phone}")  # Out: 5555555555
    john.remove_phone("5555555555")

    # Delete Jane entry
    book.delete("Jane")

    # List all entries in the address book in their final stage
    book.print_book()


def db_flow(reset_data: bool):
    with AddressBookReader() as book:
        # Show initial book state
        book.print_book()
        print(reset_data)
        if not book.data or reset_data:
            print("--------- First run ---------")
            ...
            try:
                ...
                john_record.edit_phone("1234567890", "3214567891")
                john_record.remove_phone("5555555555")
            except KeyError as e:
                print(e)

        # Show final book once script is completed.
        book.print_book()


if __name__ == "__main__":
    args = argument_parser.parse_args()
    if args.db_mode == "true":
        print("Entering script in db_mode ...")
        db_flow(args.reset_data == "true")
    else:
        print("Entering script in default mode ...")
        main_assignment_flow()
